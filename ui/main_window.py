...
from PySide6.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                               ...
                               QSizePolicy, QDateEdit, QGridLayout)
from PySide6.QtCore import Qt, QRegularExpression, QDate, QThread, Signal
...
from modules.shorts_studio import ShortsStudioWidget
from modules.youtube_downloader import YouTubeDownloaderWidget
...
class ChatGptBridgeTestWorker(QThread):
    ...
    def __init__(self, mode, prompt="", parent=None):
    ...
    def run(self):
        bridge = ChatGptBrowserBridge(log=self.status.emit)
        try:
            if self.mode == "open":
                bridge.open_ready_session(on_login_required=self._request_login)
                self.response.emit("ChatGPT session is ready.")
            else:
                ...
                self.response.emit(result.get("text", "") or json.dumps(result, ensure_ascii=False, indent=2))
...
I18N = {
    "tr": {
        ...
        "llm_provider_lm_studio": "LM Studio",
        "endpoint_label": "Endpoint:",
        "model_label": "Model:",
        "flm_required_status": "FLM seçili. Kayıt için FLM servisini başlatın veya Standart Whisper seçin.",
        ...
        "about_text": "<b>Local AI Suite</b><br><br>Bu uygulama <b>AMD NPU</b> kullanılarak geliştirilmiştir.<br><br>Geliştirici Serhat Youtube kanalı da Teknoloji ve Hayat: <a href='https://www.youtube.com/@TeknolojiHayat' style='color:#89B4FA;'>https://www.youtube.com/@TeknolojiHayat</a>",
        # Rich text editor
        "rte_color": "🎨 Renk",
    ...
    "en": {
        ...
        "endpoint_label": "Endpoint:",
        "model_label": "Model:",
        "flm_required_status": "FLM selected. Start FLM service for recording or choose Standard Whisper.",
        ...
        "btn_about": "❓ About",
        "about_title": "About",
        "about_text": "<b>Local AI Suite</b><br><br>This application was developed using <b>AMD NPU</b>.<br><br>Developer Serhat YouTube Channel Teknoloji ve Hayat: <a href='https://www.youtube.com/@TeknolojiHayat' style='color:#89B4FA;'>https://www.youtube.com/@TeknolojiHayat</a>",
        # Rich text editor
        "rte_color": "🎨 Color",
...
class HistoryDialog(QDialog):
    def __init__(self, db, lang="tr", parent=None, flm_manager=None):
        ...
        self.detail_title_edit.setObjectName("meeting_title_edit")
        self.detail_title_edit.setPlaceholderText(t["history_detail_label"])
        self.detail_title_edit.setEnabled(False)
        self.detail_title_edit.textEdited.connect(lambda _: self.btn_save.setEnabled(True))
        self.detail_date_label = QLabel("")
        self.detail_date_label.setObjectName("meeting_title_label")
        title_layout.addWidget(self.detail_title_edit, 1)
        title_layout.addWidget(self.detail_date_label)
    ...
    def on_meeting_selected(self):
        ...
        if record:
            self.current_title = record[0] or ""
            self.detail_title_edit.setEnabled(True)
            self.detail_title_edit.setText(self.current_title)
            self.detail_title_edit.setModified(False)
            self.detail_date_label.setText(str(record[1] or ""))
            self.participants_edit.setText(record[4] or "")
            self.participants_edit.setModified(False)
    ...
    def _has_unsaved_changes(self):
        return (self.detail_title_edit.isModified() or
                self.participants_edit.isModified() or
                self.summary_editor.is_modified() or
                self.transcript_editor.is_modified())

    def save_current(self):
        if self.current_meeting_id is None:
            return
        title = self.detail_title_edit.text().strip() or self.current_title
        ...
        self.detail_title_edit.setText(title)
        self.detail_title_edit.setModified(False)
        self.participants_edit.setModified(False)
        self.summary_editor.set_modified(False)
        self.transcript_editor.set_modified(False)
    ...
    def reanalyze_current(self):
        ...
        llm_config = self._get_selected_llm_config()
        if llm_config["provider"] == "flm" and self.flm_manager:
            self._pending_reanalyze_transcript = transcript
            self.flm_manager.start_llm_service(llm_config["model_name"])
            return
    ...
    def _get_selected_llm_config(self):
        parent = self.parent()
        if parent and hasattr(parent, "get_selected_llm_config"):
            return parent.get_selected_llm_config()
        return {
            "provider": "flm",
            "base_url": LlmAnalyzerThread.PROVIDER_DEFAULTS["flm"]["base_url"],
    ...
    def _on_reanalyze_done(self, title, participants, summary):
        ...
        self.btn_save.setEnabled(True)
        self.load_meetings()
        self._restore_parent_llm_service()
    ...
    def _restore_parent_llm_service(self):
        parent = self.parent()
        if parent and hasattr(parent, "_restore_after_llm_analysis"):
            parent._restore_after_llm_analysis()
        elif self.flm_manager:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setStyleSheet("""
            ...
            QTabBar::tab { background: #181825; color: #A6ADC8; padding: 8px 18px; border-radius: 4px 4px 0 0; margin-right: 2px; }
            QTabBar::tab:selected { background: #313244; color: #CDD6F4; }
            QTabBar::tab:hover { background: #45475A; }
        """)
        ...
        self.llm_base_url = self.db.get_setting(
            "llm_base_url",
            LlmAnalyzerThread.PROVIDER_DEFAULTS.get(self.llm_provider, LlmAnalyzerThread.PROVIDER_DEFAULTS["flm"])["base_url"],
        )
        ...
        self.model_session_manager = ModelSessionManager(self.flm_manager, self._is_note_taker_busy_for_model)
    ...
    def _build_module_registry(self):
        """Small local registry for the first Local AI Suite modules."""
        return [
            {
                "module_id": "note_taker",
                "title": "Note Taker",
                "description": "Local meeting recording, transcription and notes.",
                "status": "idle",
                "render": self._render_note_taker_module,
            },
            {
                "module_id": "shorts_studio",
                "title": "Shorts Studio",
                "description": "YouTube linkinden otomatik shorts uretme modulu burada olacak.",
                "status": "idle",
                "render": self._render_shorts_studio_module,
            },
            {
                "module_id": "youtube_downloader",
                "title": "YouTube Downloader",
                "description": "Tek video, playlist veya MP3 indirme.",
                "status": "idle",
                "render": self._render_youtube_downloader_module,
            },
            {
                "module_id": "pc_assistant",
                "title": "PC Assistant",
                "description": "Ekran, browser ve PowerShell destekli yerel PC asistani burada olacak.",
                "status": "idle",
                "render": self._render_pc_assistant_module,
            },
            {
                "module_id": "settings_logs",
                "title": "Settings / Logs",
                "description": "Application settings and recent local logs.",
                "status": "idle",
                "render": self._render_settings_logs_module,
            },
        ]
    ...
    def _set_module_status(self, module_id, status):
        ...
        if module:
            ...
            self._refresh_settings_logs()

    def _update_note_taker_status(self):
        if getattr(self, "_pending_live_analysis", False) or getattr(self, "_finishing", False) or self.meeting_active:
            self._set_module_status("note_taker", "running")
        elif self.llm_thread and self.llm_thread.isRunning():
            self._set_module_status("note_taker", "running")
        else:
            self._set_module_status("note_taker", "idle")

    def _is_note_taker_busy_for_model(self):
        if self.meeting_active or getattr(self, "_finishing", False) or getattr(self, "_pending_live_analysis", False):
            return True
        return bool(self.llm_thread and self.llm_thread.isRunning())

    def _render_note_taker_module(self):
        return self.note_taker_tab

    def _render_placeholder_module(self, title, body):
        widget = QWidget()
    ...
    def _render_shorts_studio_module(self):
        self.shorts_studio_widget = ShortsStudioWidget(
            model_session_manager=self.model_session_manager,
            default_model=self.flm_model_name,
            parent=self,
        )
        return self.shorts_studio_widget

    def _render_youtube_downloader_module(self):
        self.youtube_downloader_widget = YouTubeDownloaderWidget(parent=self)
        return self.youtube_downloader_widget

    def _render_pc_assistant_module(self):
        return self._render_placeholder_module(
            "PC Assistant",
            "Ekran, browser ve PowerShell destekli yerel PC asistanı burada olacak.",
        )

    def _render_settings_logs_module(self):
        widget = QWidget()
        layout = QVBoxLayout(widget)
        layout.setContentsMargins(24, 24, 24, 24)
        layout.setSpacing(12)
        ...
        self.settings_info_label.setStyleSheet("color: #CDD6F4; font-size: 13px;")
        ...
        btn_refresh_logs.clicked.connect(self._refresh_settings_logs)
        logs_layout.addWidget(btn_refresh_logs)

        bridge_group = QGroupBox("ChatGPT Browser Bridge")
        bridge_layout = QVBoxLayout(bridge_group)
        bridge_buttons = QHBoxLayout()
        self.btn_open_chatgpt_bridge = QPushButton("Open ChatGPT Chrome Session")
        self.btn_open_chatgpt_bridge.clicked.connect(self._open_chatgpt_bridge_session)
        self.btn_send_chatgpt_test = QPushButton("Send Test Prompt")
        self.btn_send_chatgpt_test.clicked.connect(self._send_chatgpt_bridge_test_prompt)
        bridge_buttons.addWidget(self.btn_open_chatgpt_bridge)
        bridge_buttons.addWidget(self.btn_send_chatgpt_test)
        bridge_buttons.addStretch()
        self.chatgpt_bridge_status = QLabel("Idle")
        self.chatgpt_bridge_prompt = QTextEdit()
        self.chatgpt_bridge_prompt.setPlaceholderText("Type a test prompt for ChatGPT Browser Bridge.")
        self.chatgpt_bridge_prompt.setMaximumHeight(90)
        self.chatgpt_bridge_response = QTextEdit()
        self.chatgpt_bridge_response.setPlaceholderText("ChatGPT response will appear here.")
        self.chatgpt_bridge_response.setMaximumHeight(120)
        bridge_layout.addLayout(bridge_buttons)
        bridge_layout.addWidget(self.chatgpt_bridge_status)
        bridge_layout.addWidget(self.chatgpt_bridge_prompt)
        bridge_layout.addWidget(self.chatgpt_bridge_response)
    ...
    def _open_chatgpt_bridge_session(self):
        self._start_chatgpt_bridge_worker("open")

    def _send_chatgpt_bridge_test_prompt(self):
        prompt = self.chatgpt_bridge_prompt.toPlainText().strip()
        if not prompt:
            prompt = "Reply with one short sentence confirming the ChatGPT Browser Bridge is working."
            self.chatgpt_bridge_prompt.setPlainText(prompt)
        self._start_chatgpt_bridge_worker("prompt", prompt)

    def _start_chatgpt_bridge_worker(self, mode, prompt=""):
        if getattr(self, "chatgpt_bridge_worker", None) and self.chatgpt_bridge_worker.isRunning():
            return
        self.chatgpt_bridge_worker = ChatGptBridgeTestWorker(mode, prompt, self)
        self.chatgpt_bridge_worker.status.connect(self._on_chatgpt_bridge_status)
        self.chatgpt_bridge_worker.response.connect(self._on_chatgpt_bridge_response)
        self.chatgpt_bridge_worker.failed.connect(self._on_chatgpt_bridge_failed)
        self.chatgpt_bridge_worker.login_required.connect(self._on_chatgpt_bridge_login_required)
        self.chatgpt_bridge_worker.finished.connect(self._on_chatgpt_bridge_finished)
        self._set_chatgpt_bridge_controls_enabled(False)
        self.chatgpt_bridge_status.setText("Starting...")
        self.chatgpt_bridge_worker.start()

    def _set_chatgpt_bridge_controls_enabled(self, enabled):
        if hasattr(self, "btn_open_chatgpt_bridge"):
            self.btn_open_chatgpt_bridge.setEnabled(enabled)
        if hasattr(self, "btn_send_chatgpt_test"):
            self.btn_send_chatgpt_test.setEnabled(enabled)
        if hasattr(self, "chatgpt_bridge_prompt"):
            self.chatgpt_bridge_prompt.setEnabled(enabled)

    def _on_chatgpt_bridge_login_required(self, message):
        QMessageBox.information(self, "ChatGPT Login", message)
        if getattr(self, "chatgpt_bridge_worker", None):
            self.chatgpt_bridge_worker.continue_after_login()

    def _on_chatgpt_bridge_status(self, message):
        if hasattr(self, "chatgpt_bridge_status"):
            self.chatgpt_bridge_status.setText(message)

    def _on_chatgpt_bridge_response(self, text):
        if hasattr(self, "chatgpt_bridge_response"):
            self.chatgpt_bridge_response.setPlainText(text)
        if hasattr(self, "chatgpt_bridge_status"):
            self.chatgpt_bridge_status.setText("Done")

    def _on_chatgpt_bridge_failed(self, message):
        if hasattr(self, "chatgpt_bridge_status"):
            self.chatgpt_bridge_status.setText(f"ERROR: {message}")
        if hasattr(self, "chatgpt_bridge_response"):
            self.chatgpt_bridge_response.setPlainText(message)

    def _on_chatgpt_bridge_finished(self):
        self._set_chatgpt_bridge_controls_enabled(True)

    def _refresh_settings_logs(self):
        if hasattr(self, "settings_info_label"):
            module_lines = [f"- {m['title']}: {m['status']}" for m in self.modules]
        ...
        if hasattr(self, "logs_box"):
            log_text = self._read_recent_error_log()
            self.logs_box.setPlainText(log_text or "No recent logs.")

    def _read_recent_error_log(self, max_lines=80):
        ...
        try:
            ...
            return f"Could not read error_log.txt: {exc}"

    def on_module_tab_changed(self, index):
        if index < 0 or index >= len(self.modules):
            return
        if self.modules[index]["module_id"] == "settings_logs":
            self._refresh_settings_logs()
    ...
    def apply_language(self):
        ...
        if hasattr(self, 'asr_provider_label'):
        ...
        if hasattr(self, 'llm_provider_label'):
            ...
            self.llm_provider_combo.setItemText(1, t["llm_provider_ollama"])
            self.llm_provider_combo.setItemText(2, t["llm_provider_lm_studio"])
        if hasattr(self, 'llm_endpoint_label'):
            self.llm_endpoint_label.setText(t["endpoint_label"])
        if hasattr(self, 'flm_model_label'):
            self.flm_model_label.setText(t["model_label"])
    ...
    def setup_ui(self):
        ...
        shell_layout = QVBoxLayout(center)
        shell_layout.setContentsMargins(20, 18, 20, 20)
        shell_layout.setSpacing(12)

        self.suite_title_label = QLabel("Local AI Suite")
        self.suite_title_label.setStyleSheet("font-size: 28px; font-weight: bold; color: #89B4FA;")
        self.suite_description_label = QLabel("Private local AI workspace for notes, automation and creative workflows.")
        self.suite_description_label.setStyleSheet("color: #A6ADC8; font-size: 13px;")
        shell_layout.addWidget(self.suite_title_label)
        shell_layout.addWidget(self.suite_description_label)

        self.module_tabs = QTabWidget()
        self.module_tabs.currentChanged.connect(self.on_module_tab_changed)
        shell_layout.addWidget(self.module_tabs, 1)

        self.note_taker_tab = QWidget()
        main_layout = QVBoxLayout(self.note_taker_tab)
        main_layout.setContentsMargins(14, 14, 14, 14)
        main_layout.setSpacing(15)

        # Top bar
        top = QHBoxLayout()
        app_title = QLabel("Note Taker")
        ...
        self.asr_provider_label.setStyleSheet("color: #BAC2DE;")
        ...
        self.asr_provider_combo.setStyleSheet(
            "background-color: #313244; color: #CDD6F4; border: 1px solid #45475A; "
            "border-radius: 4px; padding: 4px 8px;"
        )

        self.whisper_model_label = QLabel()
        self.whisper_model_label.setStyleSheet("color: #BAC2DE;")
        self.whisper_model_input = QLineEdit()
        self.whisper_model_input.setText(self.whisper_model_name)
        self.whisper_model_input.setPlaceholderText("small")
        self.whisper_model_input.setFixedWidth(110)
        self.whisper_model_input.editingFinished.connect(self.save_provider_settings)
        self.whisper_model_input.setStyleSheet(
            "background-color: #313244; color: #CDD6F4; border: 1px solid #45475A; "
            "border-radius: 4px; padding: 4px 8px;"
        )
        ...
        self.llm_provider_combo.addItem("FLM", "flm")
        self.llm_provider_combo.addItem("Ollama", "ollama")
        self.llm_provider_combo.addItem("LM Studio", "lm_studio")
        self.llm_provider_combo.setCurrentIndex(
            max(0, self.llm_provider_combo.findData(self.llm_provider))
        )
        self.llm_provider_combo.currentIndexChanged.connect(self.on_llm_provider_changed)
        self.llm_provider_combo.setStyleSheet(
            "background-color: #313244; color: #CDD6F4; border: 1px solid #45475A; "
            "border-radius: 4px; padding: 4px 8px;"
        )

        self.llm_endpoint_label = QLabel()
        self.llm_endpoint_label.setStyleSheet("color: #BAC2DE;")
        self.llm_endpoint_input = QLineEdit()
        self.llm_endpoint_input.setText(self.llm_base_url)
        self.llm_endpoint_input.setPlaceholderText("http://127.0.0.1:11434/v1")
        self.llm_endpoint_input.setFixedWidth(210)
        self.llm_endpoint_input.editingFinished.connect(self.save_provider_settings)
        self.llm_endpoint_input.setStyleSheet(
            "background-color: #313244; color: #CDD6F4; border: 1px solid #45475A; "
            "border-radius: 4px; padding: 4px 8px;"
        )
        ...
        self.flm_model_input.setText(self.llm_model_name)
        self.flm_model_input.setPlaceholderText("qwen3.5:4b")
        self.flm_model_input.setFixedWidth(180)
        self.flm_model_input.editingFinished.connect(self.save_provider_settings)
        ...
        service_layout.addWidget(self.status_indicator, 0, 0, 1, 2)
        service_layout.addWidget(self.asr_provider_label, 0, 2)
        service_layout.addWidget(self.asr_provider_combo, 0, 3)
        service_layout.addWidget(self.whisper_model_label, 0, 4)
        service_layout.addWidget(self.whisper_model_input, 0, 5)
        service_layout.addWidget(self.btn_start_flm, 0, 6)
        service_layout.addWidget(self.llm_provider_label, 1, 0)
        service_layout.addWidget(self.llm_provider_combo, 1, 1)
        service_layout.addWidget(self.llm_endpoint_label, 1, 2)
        ...
        service_layout.addLayout(ics_layout, 2, 0, 1, 8)
        ...
        for module in self.modules:
            self.module_tabs.addTab(module["render"](), module["title"])
    ...
    def _sync_capture_control_states(self):
        ...
        if hasattr(self, "asr_provider_combo"):
            ...
            self.flm_model_input.setEnabled(
                provider_inputs_enabled and not (
                    self.flm_manager.is_running and self.flm_manager.service_mode == "llm"
                )
            )

    def _default_llm_base_url(self, provider):
        return LlmAnalyzerThread.PROVIDER_DEFAULTS.get(
            provider,
            LlmAnalyzerThread.PROVIDER_DEFAULTS["flm"],
        )["base_url"]

    def _default_llm_model(self, provider):
        if provider == "flm":
            return self.flm_model_name or self.flm_manager.DEFAULT_MODEL
        return LlmAnalyzerThread.PROVIDER_DEFAULTS.get(
            provider,
            LlmAnalyzerThread.PROVIDER_DEFAULTS["flm"],
        )["model"]

    def get_selected_asr_provider(self):
        if hasattr(self, "asr_provider_combo"):
            return self.asr_provider_combo.currentData() or "flm"
        return self.asr_provider or "flm"

    def get_selected_whisper_model(self):
        if hasattr(self, "whisper_model_input"):
            model_name = self.whisper_model_input.text().strip()
        else:
    ...
    def get_selected_llm_provider(self):
        ...
        return self.llm_provider or "flm"

    def get_selected_llm_base_url(self):
        provider = self.get_selected_llm_provider()
        if hasattr(self, "llm_endpoint_input"):
            base_url = self.llm_endpoint_input.text().strip()
        else:
            base_url = self.llm_base_url
        return base_url or self._default_llm_base_url(provider)
    ...
    def get_selected_llm_model(self):
        ...
        return model_name or self._default_llm_model(provider)

    def get_selected_llm_config(self):
        return {
            "provider": self.get_selected_llm_provider(),
            "base_url": self.get_selected_llm_base_url(),
            "model_name": self.get_selected_llm_model(),
        }
    ...
    def on_llm_provider_changed(self, *_):
        provider = self.get_selected_llm_provider()
        self.llm_endpoint_input.setText(self._default_llm_base_url(provider))
        self.flm_model_input.setText(self._default_llm_model(provider))
        self.save_provider_settings()
        if provider in ("ollama", "lm_studio"):
            self._prompt_missing_provider_install(provider)

    def _prompt_missing_provider_install(self, provider):
        if self._provider_dependency_ready(provider):
            return
    ...
    def _provider_dependency_ready(self, provider):
        ...
        if provider == "lm_studio":
            return check_lm_studio_installed()
        return True
    ...
    def save_provider_settings(self, *_):
        ...
        if self.llm_provider == "flm":
            self.flm_model_name = self.llm_model_name
            self.db.save_setting("flm_model_name", self.flm_model_name)

        if hasattr(self, "shorts_studio_widget"):
            self.shorts_studio_widget.default_model = self.flm_model_name
            if not self.shorts_studio_widget.model_input.text().strip():
                self.shorts_studio_widget.model_input.setText(self.flm_model_name)
    ...
    def _recording_provider_ready(self):
        return self.get_selected_asr_provider() != "flm" or self.flm_manager.is_ready

    def _sync_provider_status(self):
        if not hasattr(self, "record_group"):
            return
        if self._recording_provider_ready():
        ...
        if self.get_selected_llm_provider() != "flm":
            provider_name = self.llm_provider_combo.currentText() if hasattr(self, "llm_provider_combo") else self.llm_provider
            self.statusBar_widget.showMessage(self.t("external_llm_status").format(provider=provider_name))
        self._sync_capture_control_states()
    ...
    def _check_asr_and_start_llm(self):
        ...
        if getattr(self, '_finishing', False):
            ...
            analyzing_msg = self.t('analyzing').format(model=llm_config["model_name"])
            ...
            self.record_group.setEnabled(False)
            self._sync_capture_control_states()
            if llm_config["provider"] == "flm":
                self.flm_manager.start_llm_service(llm_config["model_name"])
            else:
                self._start_live_analysis_thread()
    ...
    def _start_live_analysis_thread(self):
        self._pending_live_analysis = False
    ...
    def _restore_after_llm_analysis(self):
        if self.get_selected_llm_provider() == "flm":
            if self.get_selected_asr_provider() == "flm":
                self.flm_manager.restart_asr_service()
            else:
    ...
    def on_analysis_completed(self, title, participants, summary):
        ...
        self._set_module_status("note_taker", "idle")
        self._restore_after_llm_analysis()

    def on_analysis_error(self, err_msg):
        self._set_module_status("note_taker", "error")
